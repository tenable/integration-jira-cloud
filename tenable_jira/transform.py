...
class Tio2Jira:
    ...
    def __init__(self, src, jira, config, update_jira_status_to_tenable=False, batch_size=10):
    ...
    def put_batch_call(self,status_changed_issues_batch):
        ...
        self._log.info("**************************************************")
        for issue in status_changed_issues_batch:
            res.append({
    ...
    def put_artifacts(self, status_changed_issues, batch_size):
        '''
        ...
        if status_changed_issues:
            for vuln in status_changed_issues:
    ...
    def post_artifacts(self, new_vuln, batch_size):
        '''
        self._src.tenable_post_artifacts.vulns(status_changed_issues)
        '''
    ...
    def ingest(self, observed_since, first_discovery):
        ...
        if isinstance(self._src, TenableIO):
            ...
            if self.update_jira_status_to_tenable:
                response_batches = self.post_artifacts(self.new_vuln, self.batch_size)
                self.upsert_finding_id_and_artifact_id(response_batches)
            ...
            if self.update_jira_status_to_tenable:
                ...
                self.put_artifacts(status_changed_issues_with_finding_id, self.batch_size)
                # Post call for the staus changed Jiras which don't have finding_id.
                response_batches = self.post_artifacts(status_changed_issues_without_finding_id, self.batch_size)
