...
class TaskInstance:
    ...
    def __init__(self, issue_def: "Task", is_open: bool = True):
        self.idef = issue_def
        self.fields = {
            'project': {'key': self.idef.jira_project_key},
            'issuetype': {'id': self.idef.jira_issue_type_id},
        }
        self.jql = [
            'project = "{self.idef.jira_project_key"',
            'issuetype = "{self.idef.jira_issue_type_name"'
        ]
        self.is_open = is_open

    def priority(self, field_id: str) -> None:
        """
        ...
        self.fields['priority'] = self.idef.severity_map[value]

    def state(self, field_id: str):
        """
...
class Task:
    ...
    def __init__(self,
        ...
        self.search = config['summary'][platform]
    ...
    def gen_description(self, finding: dict) -> dict:
        ...
        """
        content = []
        for title, attr in self.description:
            content.append({
                'type': 'heading',
                'attrs': {'level': 1},
                'content': [{
            ...
            content.append({
                ...
                'content': [{
                    'type': 'text',
                    'text': trunc(finding.get(attr, 'No Output'),
                                  limit=10000,
                                  suffix='..'
                                  )
                }]
