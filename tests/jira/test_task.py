...
from tenb2jira.jira.task import Task
...
def test_description():
    return {
        ...
        "content": [
            ...
            {
                "type": "paragraph",
                "content": [{"type": "text", "text": "Test Solution"}],
            },
        ],
    }
...
def test_task_noapi(jira_config, task_config, test_fields):
    task = Task(
        config=task_config, jira_config=jira_config, platform="tvm", fields=test_fields
    )
    assert task.id == 1
    ...
    del task_config["id"]
    project = Box({"issueTypes": [{"name": "Task", "id": 1}]})
    task = Task(
        config=task_config,
    ...
    project = Box({"issueTypes": [{"name": "Sub-task", "id": 1}]})
...
def test_gen_description(jira_config, task_config, test_fields, test_description):
    task = Task(
        config=task_config, jira_config=jira_config, platform="tvm", fields=test_fields
    )
    finding = {"description": "Test Description", "solution": "Test Solution"}
...
def test_generate(jira_config, task_config, test_fields, test_description):
    task = Task(
        config=task_config, jira_config=jira_config, platform="tvm", fields=test_fields
    )
    finding = {
    ...
    jql = 'project = "VULN" AND issuetype = "Task" AND "Test Field" ~ "something"'
...
def test_with_tvm_data(tvm_generator, example_config):
    ...
    fields = [
        Field(f, platform="tvm", platform_map=example_config["tenable"]["platforms"])
        for f in example_config["jira"]["fields"]
    ]
    task = Task(
        config=example_config["jira"]["task"],
        jira_config=example_config["jira"],
        platform="tvm",
        fields=fields,
    )
    subtask = Task(
        config=example_config["jira"]["subtask"],
        jira_config=example_config["jira"],
        platform="tvm",
        fields=fields,
    )
...
def test_with_tsc_data(tsc_generator, example_config):
    fields = [
        Field(f, platform="tsc", platform_map=example_config["tenable"]["platforms"])
        for f in example_config["jira"]["fields"]
    ]
    task = Task(
        config=example_config["jira"]["task"],
    ...
    assert repr(rtask) == (
        'Task("project = "VULN" AND issuetype = "Task" AND '
