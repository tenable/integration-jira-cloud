...
def test_transition(jiraapi):
    responses.post('https://nourl/rest/api/3/issue/1/transitions',
...
def test_update(jiraapi):
    responses.put('https://nourl/rest/api/3/issue/1',
                  match=[
...
def test_create(jiraapi):
    responses.post('https://nourl/rest/api/3/issue',
                   match=[
                       matchers.json_params_matcher({'test': 'value'}),
                       matchers.query_param_matcher({
...
def test_list_no_iter(jiraapi):
    ...
    jiraapi.issues.search(jql='test jql search',
                          fields=['id'],
                          use_iter=False
                          )
...
def test_list_iter(jiraapi):
    ...
    iter_obj = jiraapi.issues.search(jql='test jql search',
                                     fields=['id'],
                                     )
    assert isinstance(iter_obj, JiraIterator)
    assert iter_obj._method == 'POST'
    assert iter_obj._envelope == 'issues'
    assert iter_obj.path == 'search'
