...
from .iterator import JiraSearchIterator
...
class IssuesAPI(APIEndpoint):
    ...
    def search(
        ...
        use_iter: bool = True,
        **kwargs,
    ) -> JiraSearchIterator | dict:
        if not expand:
            expand = []
        kwargs["jql"] = jql
        kwargs["expand"] = ",".join(expand)
        kwargs["fields"] = fields
        if use_iter:
            return JiraSearchIterator(
                self._api,
                _method="POST",
                _envelope="issues",
                path="search/jql",
    ...
    def create(self, update_history: bool = False, **kwargs) -> Dict:
        return self._post(json=kwargs, params={"update_history": update_history})
