...
class Task:
    ...
    def __init__(
        ...
        if not self.fetch_issue_id(project):
            raise AttributeError(
                (f"Could not determine Issue Type id for {self.name}:{self.type}")
            )
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": trunc(
                                finding.get(item["attr"], "No Output"),
                                limit=10000,
                                suffix="...",
                            ),
                        }
    ...
    def gen_summary(self, finding: dict) -> str:
        ...
        """
        return trunc(self.summary.format(f=finding), limit=255, suffix="...")

    def generate(self, finding: dict, is_open: Optional[bool] = None) -> TaskInstance:
        """
