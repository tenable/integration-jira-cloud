...
from .generators import tsc_merged_data, tvm_asset_cleanup, tvm_merged_data
...
class Tenable:
    ...
    def __init__(self, config: dict):
        ...
        self.close_accepted = self.config["tenable"].get("fix_accepted_risks", True)
        self.vpr_score = self.config["tenable"].get("vpr_score")
        self.tags = self.config["tenable"].get("tags")

        if not self.timestamp:
            self.timestamp = int(
                arrow.now().shift(days=-self.age).floor("day").timestamp()
            )
    ...
    def get_tsc_generator(self, start_time: int) -> Generator[Any, Any, Any]:
        ...
        params = {"source": "cumulative", "limit": self.page_size}
        ...
        return tsc_merged_data(
            cumulative,
            patched,
            close_accepted=self.close_accepted,
        )

    def get_generator(self, start_time: arrow.Arrow) -> Generator[Any, Any, Any]:
        """
        Retrieves the appropriate generator based on the configured platform.
        """
        if self.platform == "tvm":
    ...
    def get_asset_cleanup(self) -> Generator[Any, Any, Any] | list:
        if self.platform == "tvm":
            dassets = self.tvm.exports.assets(
                deleted_at=self.timestamp, chunk_size=self.chunk_size
            )
            tassets = self.tvm.exports.assets(
                terminated_at=self.timestamp, chunk_size=self.chunk_size
            )
